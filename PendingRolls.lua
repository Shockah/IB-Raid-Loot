...
function IBRaidLoot:ShowPendingRollsFrame()
	if not Frame then
...
function IBRaidLoot:CreatePendingRollsFrame()
	if Frame then
		self:DebugPrint("Already created Pending Rolls frame.")
		return
	end
...
function IBRaidLoot:UpdatePendingRollsFrame(closeIfNoItems, onlyRollCounts)
	if not Frame or not Frame:IsVisible() then
		return
	end

	self:FreePendingRollsItemFrames()
...
function IBRaidLoot:FreePendingRollsItemFrames()
	for _, frame in pairs(ItemsFrame.subframeCache) do
		frame:Hide()
	end
